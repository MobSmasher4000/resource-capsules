...
public class ModBlockStateProvider extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        ...
        fluidGenMultiblockTexture(ModBlocks.FLUID_GEN_MULTIBLOCK.get());
    ...
    private void fluidGenMultiblockTexture(Block block) {
        // Gets the exact registry name (e.g., "resource_gen_tier_3")
        String name = BuiltInRegistries.BLOCK.getKey(block).getPath();
        ...
        getVariantBuilder(block).forAllStates(state -> {
            Direction dir = state.getValue(BlockStateProperties.HORIZONTAL_FACING);
            boolean isFormed = state.getValue(FluidGenMultiblockBlock.FORMED);
