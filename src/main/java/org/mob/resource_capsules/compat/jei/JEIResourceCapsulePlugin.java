...
public class JEIResourceCapsulePlugin implements IModPlugin {
    ...
    public ResourceLocation getPluginUid() {
        return ResourceLocation.fromNamespaceAndPath(ResourceCapsules.MOD_ID, "jei_plugin");
    }
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        List<ResourceGenTier1Recipe> resourceGenTier1Recipes = recipeManager.getAllRecipesFor(ResourceGenTier1Recipe.Type.INSTANCE).stream().map(RecipeHolder::value).toList();
        registration.addRecipes(ResourceGenTier1Category.RESOURCE_GEN_TIER_1_RECIPE_TYPE, resourceGenTier1Recipes);

        List<ResourceGenTier2Recipe> resourceGenTier2Recipes = recipeManager.getAllRecipesFor(ResourceGenTier2Recipe.Type.INSTANCE).stream().map(RecipeHolder::value).toList();
        registration.addRecipes(ResourceGenTier2Category.RESOURCE_GEN_TIER_2_RECIPE_TYPE, resourceGenTier2Recipes);

        List<ResourceGenTier3Recipe> resourceGenTier3Recipes = recipeManager.getAllRecipesFor(ResourceGenTier3Recipe.Type.INSTANCE).stream().map(RecipeHolder::value).toList();
        registration.addRecipes(ResourceGenTier3Category.RESOURCE_GEN_TIER_3_RECIPE_TYPE, resourceGenTier3Recipes);

        List<DimensionalResourceGenRecipe> dimensionalResourceGenRecipes = recipeManager.getAllRecipesFor(DimensionalResourceGenRecipe.Type.INSTANCE).stream().map(RecipeHolder::value).toList();
        ...
        List<EncapsulatingTransmutatorRecipe> encapsulatingTransmutatorRecipes = recipeManager.getAllRecipesFor(EncapsulatingTransmutatorRecipe.Type.INSTANCE).stream().map(RecipeHolder::value).toList();
        ...
        List<CatalyticConverterRecipe> catalyticConverterRecipes = recipeManager.getAllRecipesFor(CatalyticConverterRecipe.Type.INSTANCE).stream().map(RecipeHolder::value).toList();
        registration.addRecipes(CatalyticConverterCategory.CATALYTIC_CONVERTER_RECIPE_TYPE, catalyticConverterRecipes);

        List<BioResouceGenRecipe> bioResouceGenRecipes = recipeManager.getAllRecipesFor(BioResouceGenRecipe.Type.INSTANCE).stream().map(RecipeHolder::value).toList();
        registration.addRecipes(BioResourceGenCategory.BIO_RESOUCE_GEN_RECIPE_TYPE, bioResouceGenRecipes);

        List<FluidGenRecipe> fluidGenRecipes = recipeManager.getAllRecipesFor(FluidGenRecipe.Type.INSTANCE).stream().map(RecipeHolder::value).toList();
